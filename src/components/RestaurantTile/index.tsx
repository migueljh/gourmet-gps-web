...
import { RatingStarIcon } from '../../Icons/restaurant.icons';

import styles from './restaurant-tile.module.scss';

const RestaurantTile = ({ name, alias, rating, image_url, categories }: RestaurantInfoInterface): JSX.Element => {
  return (
    <button className={styles.restaurantTileContainer}>
      <Link to={`/restaurant/${alias}`} title={`View details of ${name}`}>
        ...
        <div className={styles.restaurantTileCategoriesSticker}>{categories[0].title}</div>
