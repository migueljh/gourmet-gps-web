...
import { getRestaurantDetails, getRestaurantReviews } from '../../controllers/restaurantController';
import { useParams } from 'react-router-dom';
import styles from './restaurant-details.module.scss';
import { RestaurantDetailsInterface, ReviewInfoInterface } from '../../interfaces';
...
const RestaurantDetails: React.FC = () => {
  ...
  const [restaurantDetails, setRestaurantDetails] = useState<RestaurantDetailsInterface>(null);
  ...
  const fetchRestaurantData = async (id: string) => {
    try {
      const [details, reviews] = await Promise.all([getRestaurantDetails(id), getRestaurantReviews(id)]);
          ...
          <>
            {restaurantDetails ? (
              <div className={styles.restaurantDetailContainer}>
                <div className={styles.restaurantDetailTopContainer}>
                  <img
                    src={restaurantDetails.image_url ? restaurantDetails.image_url : '/gourmet-gps-logo.svg'}
                  ...
                  <div className={styles.restaurantDetailRestInfo}>
                    ...
                    <h1 className={styles.restaurantDetailsRestTitle}>{restaurantDetails.name}</h1>
                    ...
                    <p className={styles.restaurantDetailAddress}>{restaurantDetails.location.display_address.join(', ')}</p>
                  </div>
                </div>
              </div>
            ) : null}
            ...
            {reviews.length > 0 ? (
              <div className={styles.restaurantDetailsReviewsContainer}>
                <h1 className={styles.restaurantDetailsReviewsTitle}>Customer reviews</h1>
              ...
              <div className={styles.restaurantDetailsNoReviewsFounded}>
                <NoDataFoundComponent text={'Sorry, there are no reviews at this time'} />
              </div>
            )}
          </>
        )}
